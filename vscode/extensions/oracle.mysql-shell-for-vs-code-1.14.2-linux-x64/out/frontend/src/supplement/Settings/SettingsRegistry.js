...
const registerSettings = () => {
    ...
    registerSetting("workers.removeIdleTime", "Delay to Remove Inactive Workers", "Specifies the number of seconds that pass until the oldest inactive worker is removed. Has no effect if the " +
        "current number of active workers fall below the minimal worker count.", "number", 5, true);
    ...
    registerSetting("dbEditor.defaultEditor", "Default Editor", "Select the type of the first editor when opening a DB connection.", "choice", "notebook", false, {
        choices: [
    ...
    registerSetting("dbEditor.useMinimap", "Use the minimap if globally enabled", "Determines if notebooks in the DB editor show a minimap. Only has an effect when mini maps are globally " +
