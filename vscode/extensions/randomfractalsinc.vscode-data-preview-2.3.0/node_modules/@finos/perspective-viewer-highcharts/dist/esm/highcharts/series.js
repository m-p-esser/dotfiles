...
function row_to_series(series, sname, gname) {
  let s;
  let sidx = 0; // TODO: figure out what this does
  ...
  *[Symbol.iterator]() {
    for (let row of this.rows) {
      if (this.columns === undefined) {
        ...
        this.is_stacked = this.columns.map(value => value.substr(value.lastIndexOf(COLUMN_SEPARATOR_STRING) + 1, value.length)).filter((value, index, self) => self.indexOf(value) === index).length > 1;
      }
...
class ColumnIterator {
  constructor(columns, pivot_length) {
    ...
    this.is_stacked = this.column_names.map(value => value.substr(value.lastIndexOf(COLUMN_SEPARATOR_STRING) + 1, value.length)).filter((value, index, self) => self.indexOf(value) === index).length > 1;
...
class MakeTick {
  constructor(schema, columns) {
    this.schema = schema; // TODO: rewrite
