...
export const draw = (mode, set_config, restyle) => async function (el, view, task, end_col, end_row) {
  if (set_config) {
  ...
  try {
    if (mode === "scatter") {
      ...
      config.legend.enabled = col_pivots.length > 0;
      config.series = series;
      config.plotOptions.scatter.marker = {
        enabled: false,
        radius: 0
      };
...
HighchartsElement = (_dec = bindTemplate(template, style), _dec(_class = class HighchartsElement extends _CustomElement {
  constructor() {
    super();
    this._charts = [];
  }
