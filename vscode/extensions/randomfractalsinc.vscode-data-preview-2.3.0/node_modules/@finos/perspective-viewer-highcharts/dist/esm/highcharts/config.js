...
export function set_tick_size(config) {
  ...
  config.plotOptions.coloredScatter = {
    marker: {
      radius: new_radius
    }
  };
...
export function set_category_axis(config, axis, type, top) {
  if (type === "datetime") {
    Object.assign(config, {
      [axis]: {
        categories: top.categories.map(x => new Date(x).toLocaleString("en-us", {
          year: "numeric",
          month: "numeric",
          day: "numeric",
          hour: "numeric",
        ...
        point: {
          events: {
            click: async function click() {
              ...
              const filters = config.filter.concat(row_filters).concat(column_filters).filter(f => !!f[f.length - 1]);
              ...
              if (type === "bubble" && mode === "scatter" || type === "scatter" && mode === "scatter" || type === "scatter" && mode === "line") {
                ...
                if (is_empty(column_name)) column_names.push(stack_name);else column_names.push(column_name);
              }
              ...
              that.dispatchEvent(new CustomEvent("perspective-click", {
                bubbles: true,
                composed: true,
                detail: {
                  column_names,
