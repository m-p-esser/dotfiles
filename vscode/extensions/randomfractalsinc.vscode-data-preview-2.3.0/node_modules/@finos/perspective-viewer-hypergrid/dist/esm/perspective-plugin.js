...
function setColumnPropsByType(column) {
  ...
  if (column.index === this.treeColumnIndex) {
    ...
    props.format = "perspective-".concat(column.type);
  }
...
function formatColumnHeader(value) {
  ...
  if (index > -1) {
    ...
    if (direction in this.charMap) {
      value = "".concat(value, " ").concat(this.charMap[direction]).concat(config.sort.length > 1 ? superscript(index + 1) : "");
    }
  }
...
export const install = function install(grid) {
