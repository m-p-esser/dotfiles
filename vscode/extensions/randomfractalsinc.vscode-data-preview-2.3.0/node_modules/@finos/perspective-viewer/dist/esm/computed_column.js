...
import { html, render } from "lit-html"; // Computations

const hour_of_day = function hour_of_day(val) {
  return new Date(val).getHours();
};

const day_of_week = function day_of_week(val) {
  return ["1 Sunday", "2 Monday", "3 Tuesday", "4 Wednesday", "5 Thursday", "6 Friday", "7 Saturday"][new Date(val).getDay()];
};

const month_of_year = function month_of_year(val) {
...
const second_bucket = function second_bucket(val) {
  return new Date(Math.floor(new Date(val).getTime() / 1000) * 1000);
};

const minute_bucket = function minute_bucket(val) {
...
const hour_bucket = function hour_bucket(val) {
  let date = new Date(val);
  date.setMinutes(0);
  date.setSeconds(0);
  date.setMilliseconds(0);
...
const day_bucket = function day_bucket(val) {
  let date = new Date(val);
  date.setHours(0);
  date.setMinutes(0);
  date.setSeconds(0);
...
const week_bucket = function week_bucket(val) {
  let date = new Date(val);
  let day = date.getDay();
...
const month_bucket = function month_bucket(val) {
  let date = new Date(val);
  date.setHours(0);
  date.setMinutes(0);
  date.setSeconds(0);
...
const year_bucket = function year_bucket(val) {
  let date = new Date(val);
  date.setHours(0);
  date.setMinutes(0);
  date.setSeconds(0);
...
export const COMPUTATIONS = {
  hour_of_day: new Computation("Hour of Day", x => "hour_of_day(".concat(x, ")"), "datetime", "integer", hour_of_day, ["Time"]),
  day_of_week: new Computation("Day of Week", x => "day_of_week(".concat(x, ")"), "datetime", "string", day_of_week, ["Time"]),
  month_of_year: new Computation("Month of Year", x => "month_of_year(".concat(x, ")"), "datetime", "string", month_of_year, ["Time"]),
  second_bucket: new Computation("Bucket (s)", x => "second_bucket(".concat(x, ")"), "datetime", "datetime", second_bucket, ["Time"]),
  minute_bucket: new Computation("Bucket (m)", x => "minute_bucket(".concat(x, ")"), "datetime", "datetime", minute_bucket, ["Time"]),
  hour_bucket: new Computation("Bueckt (h)", x => "hour_bucket(".concat(x, ")"), "datetime", "datetime", hour_bucket, ["Time"]),
  day_bucket: new Computation("Bucket (D)", x => "day_bucket(".concat(x, ")"), "datetime", "date", day_bucket, ["Time"]),
  week_bucket: new Computation("Bucket (W)", x => "week_bucket(".concat(x, ")"), "datetime", "date", week_bucket, ["Time"]),
  month_bucket: new Computation("Bucket (M)", x => "month_bucket(".concat(x, ")"), "datetime", "date", month_bucket, ["Time"]),
  year_bucket: new Computation("Bucket (Y)", x => "year_bucket(".concat(x, ")"), "datetime", "date", year_bucket, ["Time"]),
  "10_bucket": new Computation("Bucket (10)", x => "bin10(".concat(x, ")"), "float", "float", x => Math.floor(x / 10) * 10, ["Math"]),
  "100_bucket": new Computation("Bucket (100)", x => "bin100(".concat(x, ")"), "float", "float", x => Math.floor(x / 100) * 100, ["Math"]),
  "1000_bucket": new Computation("Bucket (1000)", x => "bin1000(".concat(x, ")"), "float", "float", x => Math.floor(x / 1000) * 1000, ["Math"]),
  "0.1_bucket": new Computation("Bucket (1/10)", x => "bin10th(".concat(x, ")"), "float", "float", x => Math.floor(x / 0.1) * 0.1, ["Math"]),
  "0.01_bucket": new Computation("Bucket (1/100)", x => "bin100th(".concat(x, ")"), "float", "float", x => Math.floor(x / 0.01) * 0.01, ["Math"]),
  "0.001_bucket": new Computation("Bucket (1/1000)", x => "bin1000th(".concat(x, ")"), "float", "float", x => Math.floor(x / 0.001) * 0.001, ["Math"]),
  add: new Computation("+", (x, y) => "".concat(x, " + ").concat(y), "float", "float", (a, b) => a + b, ["Math"], 2),
  subtract: new Computation("-", (x, y) => "".concat(x, " - ").concat(y), "float", "float", (a, b) => a - b, ["Math"], 2),
  multiply: new Computation("*", (x, y) => "".concat(x, " * ").concat(y), "float", "float", (a, b) => a * b, ["Math"], 2),
  divide: new Computation("/", (x, y) => "".concat(x, " / ").concat(y), "float", "float", (a, b) => a / b, ["Math"], 2),
  percent_a_of_b: new Computation("%", (x, y) => "".concat(x, " %% ").concat(y), "float", "float", (a, b) => a / b * 100, ["Math"], 2),
  uppercase: new Computation("Uppercase", x => "uppercase(".concat(x, ")"), "string", "string", x => x.toUpperCase(), ["Text"]),
  lowercase: new Computation("Lowercase", x => "lowercase(".concat(x, ")"), "string", "string", x => x.toLowerCase(), ["Text"]),
  length: new Computation("length", x => "length(".concat(x, ")"), "string", "integer", x => x.length, ["Text"]),
  concat_space: new Computation("concat_space", x => "concat_space(".concat(x, ")"), "string", "string", (a, b) => a + " " + b, ["Text"], 2),
  concat_comma: new Computation("concat_comma", x => "concat_comma(".concat(x, ")"), "string", "string", (a, b) => a + ", " + b, ["Text"], 2)
};
...
let TREE = _get_tree(); // Eslint complains here because we don't do anything, but actually we globally
// register this class as a CustomElement


let // eslint-disable-next-line no-unused-vars
ComputedColumn = (_dec = bindTemplate(template, style), _dec(_class = class ComputedColumn extends _CustomElement {
  constructor() {
    super();
  ...
  *_selector_template(tree = TREE) {
    for (const [category, comp] of Object.entries(tree)) {
      if (comp.name) {
        yield html(_templateObject(), category, comp.name);
      } else {
        yield html(_templateObject2(), category, Array.from(this._selector_template(comp)));
      }
    }
  ...
  _drop_column(event) {
    ...
    const is_swap = this.state.swap_target !== undefined && target.innerHTML.indexOf("perspective-row") > -1; // column must match return type of computation
    ...
    if (is_swap) {
      ...
      event.swapTarget = this.state.swap_target; // take the column at the drop target, and set it to the column
      // being swapped

      this._set_input_column(event, current_column_name, current_column_type); // reset swap_target and currentTarget
  ...
  _pass_column(event) {
    const src = event.currentTarget; // are we inside the column? if we are, prevent further calls which
    // cause flickering
  ...
  _apply_state(columns, computation, name) {
    ...
    for (let i = 0; i < this.state["input_columns"].length; i++) {
      if (this.state["input_columns"][i] !== undefined) {
        ...
        }, this.state["input_columns"][i].name, this.state["input_columns"][i].type);
      }
    }
  ...
  _auto_column_name() {
    ...
    if (this.state.input_columns.length > 0) {
      ...
      if (this.state.computation[FORMATTER]) {
        ...
        this._column_name_input.innerText = "".concat(this.state.computation.name, "(").concat(names.join(", "), ")");
      }
    } else {
  ...
  _set_input_column(event, name, type) {
    ...
    if (computation_type !== "float" && computation_type !== "datetime" && type !== computation_type || computation_type === "float" && type !== "float" && type !== "integer" || computation_type === "datetime" && type !== "datetime" && type !== "date") {
      this._register_inputs();
  ...
  _update_computation(event, computation_name) {
    ...
    this._computation_type.innerHTML = "<span class=\"".concat(return_type, "\">").concat(this.type_markers[return_type], "</span>");
  ...
  _register_ids() {
    ...
    this._input_columns = this.shadowRoot.querySelector("#psp-cc-computation-inputs"); //this._delete_button =
...
//# sourceMappingURL=computed_column.js.map