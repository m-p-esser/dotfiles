...
export class ActionElement extends DomElement {
  ...
  async _toggle_config(event) {
    if (!event || event.button !== 2) {
      ...
      if (!this._show_config) {
        ...
        this._datavis.style.width = "".concat(panel.clientWidth + this._side_panel.clientWidth, "px");
        this._datavis.style.height = "".concat(panel.clientHeight + this._top_panel.clientHeight, "px");
  ...
  async _create_computed_column(event) {
    ...
    const cols = await this._table.columns(); // edit overwrites last column, otherwise avoid name collision

    if (cols.includes(computed_column_name)) {
      console.log(computed_column_name);
      computed_column_name += " ".concat(Math.round(Math.random() * 100));
    }
  ...
  _register_callbacks() {
    ...
    this._computed_column.addEventListener("perspective-computed-column-update", this._set_computed_column_input.bind(this)); // this._side_panel.addEventListener('
...
//# sourceMappingURL=action_element.js.map