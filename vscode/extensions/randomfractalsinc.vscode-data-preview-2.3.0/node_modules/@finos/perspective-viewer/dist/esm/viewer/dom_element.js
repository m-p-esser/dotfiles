...
export class DomElement extends PerspectiveElement {
  ...
  _set_row_type(row) {
    const weights = this._get_view_dom_columns("#inactive_columns perspective-row").filter(x => x.getAttribute("type") === "integer" || x.getAttribute("type") === "float").map(x => x.getAttribute("name"));
  ...
  _autocomplete_choices(json) {
    return json.slice(1, json.length).map(x => x.__ROW_PATH__).filter(x => Array.isArray(x) ? x.filter(v => !!v).length > 0 : !!x);
  }
