...
export default function (Module) {
  ...
  const _parse_format_options = function _parse_format_options(options) {
    ...
    const end_col = Math.min(max_cols, (options.end_col !== undefined ? options.end_col : viewport.width ? start_col + viewport.width : max_cols) * (hidden + 1)); // Return the calculated values
  ...
  _table.prototype.schema = function (computed = false, override = true) {
  ...
  _table.prototype.computed_schema = function () {
  ...
  _table.prototype.add_computed = function (computed) {
    let _Table;
    ...
    try {
      ...
      return new _table(_Table, this.index, computed, this.limit, this.overridden_types);
    } catch (e) {
      if (_Table) {
        _Table.delete();
      }
  ...
  _table.prototype.columns = function (computed = false) {
    let schema = this._Table.get_schema();
