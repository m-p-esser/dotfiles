...
exports.compressToBuffer = exports.maxCompressedLength = void 0;
const BLOCK_LOG = 16;
...
function hash32(array, pos, shift) {
    return (((array[pos]
        + (array[pos + 1] << 8)
        + (array[pos + 2] << 16)
        + (array[pos + 3] << 24)) * 0x1e35a7bd) & 0xffffffff) >>> shift;
}
...
function emitCopyAtMost64(output, op, offset, len) {
...
function compressFragment(input, baseIp, inputSize, output, baseOp) {
...
function maxCompressedLength(input) {
    const sourceLen = input.length;
    return 32 + sourceLen + Math.floor(sourceLen / 6);
}
exports.maxCompressedLength = maxCompressedLength;
function compressToBuffer(input, output) {
...
exports.compressToBuffer = compressToBuffer;
