...
function decodeDataPages(buffer, column, compression, num_values, dictionary) {
    const cursor = {
        buffer,
        offset: 0,
        size: buffer.length
...
function decodeDictionaryPage(cursor, header, column, compression) {
    const cursorEnd = cursor.offset + header.compressed_page_size;
    let dictCursor = {
        offset: 0,
    ...
    cursor.offset = cursorEnd;
    if (compression !== 'UNCOMPRESSED') {
        const valuesBuf = Compression.inflate(compression, dictCursor.buffer.slice(dictCursor.offset, cursorEnd), header.uncompressed_page_size);
        dictCursor = {
            buffer: valuesBuf,
            offset: 0,
            size: valuesBuf.length
